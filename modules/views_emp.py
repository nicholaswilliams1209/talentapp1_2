...
from modules.views_kudos import render_kudos_feed, render_send_kudos_form
...
def render_employee_view():
    ...
    t1, t2, t3, t4, t5, t6 = st.tabs(["📝 Samoprocjena", "📊 Gap Analiza", "🎯 Moji Ciljevi", "🚀 Moj IDP", "📜 Povijest", "🌟 Kudos Wall"])
    ...
    with t6:
        ...
        with kc2:
            st.subheader("✉️ Pohvali kolegu")
            my_sender_name = emp_res[0] if emp_res else username
            render_send_kudos_form(username, my_sender_name, company_id)
        with kc1:
            st.subheader("🌟 Zid pohvala")
            render_kudos_feed(username, company_id)
