...
def render_admin_view():
    ...
    with tab1:
        ...
        conn = get_connection()
        try:
            users = pd.read_sql_query("SELECT username, role, department FROM users", conn)
        finally:
            conn.close()
        st.dataframe(users)
        ...
        with st.form("panic_form"):
            ...
            if st.form_submit_button("🔓 OTKLJUČAJ (Vrati na Draft)"):
                if target_id and target_period and reason:
                    ...
                    try:
                        check = conn.execute(
                            "SELECT status FROM evaluations WHERE kadrovski_broj=? AND period=?",
                        ...
                        if not check:
                            ...
                            conn.execute(
                                "UPDATE evaluations SET status='Draft' WHERE kadrovski_broj=? AND period=?",
                                (target_id, target_period)
                            )
                            conn.commit()
                            ...
                            st.toast(f"✅ Procjena za {target_id} vraćena u Draft. Manager može uređivati.", icon="🔓")
                        st.success(f"Procjena za **{target_id}** ({target_period}) je vraćena u Draft status.")
                    finally:
                        conn.close()
                else:
    ...
    with tab3:
        ...
        if st.button("Osvježi logove"):
            ...
            try:
                ...
                st.warning("Audit log je prazan ili još nije kreiran.")
            finally:
                conn.close()
