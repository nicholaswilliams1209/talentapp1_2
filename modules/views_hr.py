...
from modules.utils import get_df_from_json, make_hashes, create_9box_grid, safe_load_json, render_empty_state, render_status_pill
...
from modules.analytics_hub import render_analytics_hub
from modules.feedback_360 import render_hr_360_admin
...
def render_hr_view():
    ...
    try:
        ...
        if menu == "📊 Analytics Hub":
            render_analytics_hub(company_id, current_period)
            ...
            if sel_emp:
                ...
                if not h.empty:
                    ...
                    render_empty_state(
                        ...
                        "Za odabranog zaposlenika još nema zaključanih (Submitted) procjena "
                        "iz prošlih ciklusa. Snail trail se gradi s vremenom, period po period."
                    )
        ...
        # ----------------------------------------------------------------
            ...
            if not filtered_master.empty:
                ...
                render_empty_state(
                    "🏢",
                    "Nema zaposlenika u odabranom odjelu",
                    "Provjerite je li odabrani odjel točan ili dodajte zaposlenike "
                    "u šifrarniku pod 'Šifarnik & Unos'.",
                    action_text="Idite na Šifarnik & Unos"
                )
        ...
        # ----------------------------------------------------------------
            ...
            if not f_m.empty:
                # PRAGMA jednom izvan petlje — izbjegavamo N ponovljenih upita
                _idp_cols = [col[1] for col in conn.execute("PRAGMA table_info(development_plans)").fetchall()]
                for _, emp in f_m.iterrows():
                    ...
                    icon = "✅" if res else "❌"
                    d_preview = dict(zip(_idp_cols, res)) if res else {}
                    idp_status_h = d_preview.get('status', 'Nije kreiran')

                    with st.expander(f"{icon} {emp['ime_prezime']} ({emp['radno_mjesto']})"):
                        if res:
                            d = dict(zip(_idp_cols, res))
                            st.markdown(render_status_pill(idp_status_h), unsafe_allow_html=True)
                            st.markdown("")
                            ...
                            render_empty_state(
                                "🚀",
                                "IDP još nije kreiran",
                                "Manager ovog zaposlenika još nije izradio individualni "
                                "razvojni plan. Podsjetite managera da krene s izradom."
                            )
            else:
                render_empty_state(
                    "👥",
                    "Nema zaposlenika za prikaz",
                    "Za odabrani odjel nema aktivnih zaposlenika. "
                    "Provjerite filtar odjela ili dodajte zaposlenike u šifrarniku.",
                    action_text="Idite na Šifarnik & Unos"
                )
        ...
        # 5. DIZAJNER UPITNIKA
        # ----------------------------------------------------------------
        elif menu == "🔄 360° Feedback Admin":
            render_hr_360_admin(company_id, current_period)
                    ...
                    if st.form_submit_button("➕ Kreiraj Predložak"):
                        if tn:
                            with sqlite3.connect(DB_FILE) as db:
                                ...
                                db.commit()
                            st.toast("✅ Predložak kreiran!", icon="📋"); st.rerun()
                ...
                if not templates_all.empty:
                    ...
                    if c_btn.button("Kopiraj"):
                        ...
                        with sqlite3.connect(DB_FILE) as db:
                            ...
                            db.commit()
                        st.toast(f"✅ Predložak kopiran: {src_tmpl} (Copy)", icon="📋"); st.rerun()
                ...
                if not templates.empty:
                    ...
                    if c_act.button("🔗 Aktiviraj za ovaj period"):
                        with sqlite3.connect(DB_FILE) as db:
                            ...
                            db.commit()
                        st.toast("✅ Upitnik aktiviran za ovaj period!", icon="🔗"); st.rerun()

                    if c_reset.button("🔄 Vrati na Default (Hardcoded)"):
                        with sqlite3.connect(DB_FILE) as db:
                            db.execute("DELETE FROM cycle_templates WHERE period_name=? AND company_id=?", (current_period, company_id))
                            db.commit()
                        st.toast("🔄 Vraćeno na tvorničke postavke!", icon="🔄"); st.rerun()
                        ...
                        if kb and ip:
                            if kb == sel_mid:
                                ...
                                with sqlite3.connect(DB_FILE) as db:
                                    ...
                                    db.commit()
                                st.toast("✅ Zaposlenik dodan!", icon="👤"); st.rerun()
            ...
            if sel_e != "---":
                    ...
                    if st.form_submit_button("Spremi Promjene"):
                        ...
                        if new_mgr_id == real_id:
                            ...
                            with sqlite3.connect(DB_FILE) as db:
                                ...
                                db.commit()
                            st.toast("✅ Podaci zaposlenika ažurirani!", icon="✏️"); st.rerun()
                ...
                with c_gdpr:
                    ...
                    if st.button("Anonimiziraj korisnika"):
                        with sqlite3.connect(DB_FILE) as db:
                            ...
                            db.commit()
                        st.toast("🕵️ Korisnik uspješno anonimiziran.", icon="🕵️"); st.rerun()
                ...
                with c_del:
                    ...
                    if st.button("TRAJNO OBRIŠI"):
                        ...
                        if res:
                            with sqlite3.connect(DB_FILE) as db:
                                db.execute("DELETE FROM employees_master WHERE kadrovski_broj=?", (real_id,))
                                db.execute("DELETE FROM users WHERE username=?", (real_id,))
                                db.execute("DELETE FROM evaluations WHERE kadrovski_broj=?", (real_id,))
                                db.execute("DELETE FROM goals WHERE kadrovski_broj=?", (real_id,))
                                db.execute("DELETE FROM development_plans WHERE kadrovski_broj=?", (real_id,))
                                db.commit()
                            st.toast("🗑️ Korisnik trajno obrisan.", icon="🗑️"); st.rerun()
                        else:
                ...
                if not periods.empty:
                    ...
                    if st.button("✅ Aktiviraj odabrano"):
                        with sqlite3.connect(DB_FILE) as db:
                            ...
                            db.commit()
                        st.toast(f"✅ Razdoblje {sel_activate} je sada aktivno!", icon="📅"); st.rerun()
                    ...
                    if st.form_submit_button("Spremi"):
                        if np:
                            with sqlite3.connect(DB_FILE) as db:
                                ...
                                db.commit()
                            st.toast("✅ Novo razdoblje kreirano!", icon="📅"); st.rerun()
                    ...
                    if st.button("🗑️ Obriši"):
                        if confirm:
                            with sqlite3.connect(DB_FILE) as db:
                                ...
                                db.commit()
                            st.toast("🗑️ Razdoblje obrisano.", icon="🗑️"); st.rerun()
