...
from modules.utils import get_df_from_json, make_hashes, create_9box_grid, safe_load_json, render_empty_state
            ...
            with t1:
                if not f_ev_mgr.empty:
                    ...
                    render_empty_state(
                        ...
                        "9-Box matrica prikazuje se kad manageri zaključaju (Submitted) procjene "
                        "za zaposlenike. Provjeri je li aktivno pravo razdoblje ili podsjetite managere.",
                        action_text="Provjeri postavke razdoblja"
                    )
            with t2:
                if not f_ev_mgr.empty:
                    ...
                    render_empty_state(
                        ...
                        "Tablica prikazuje zaposlenike s zaključanim procjenama. "
                        "Za odabrani odjel/period još nema završenih ocjena."
                    )
        ...
        # ----------------------------------------------------------------
            ...
            if not f_m.empty:
                for _, emp in f_m.iterrows():
                    ...
                    icon = "✅" if res else "❌"
                    cols_dp = [c[1] for c in conn.execute("PRAGMA table_info(development_plans)").fetchall()]
                    d_preview = dict(zip(cols_dp, res)) if res else {}
