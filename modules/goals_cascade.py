    ...
    try:
            ...
            if org_goals.empty:
                ...
                for _, og in org_goals.iterrows():
                    ...
                    linked_dept = pd.read_sql_query(
                        "SELECT * FROM goals WHERE parent_goal_id=? AND level='dept'"                        " AND company_id=?",
                        conn, params=(og['id'], company_id)
                    )
                    for _, dg in linked_dept.iterrows():
                        ...
                        # Zaposlenik ciljevi — samo zaposlenici ovog managera
                        emp_goals = pd.read_sql_query(
                            ...
                            "WHERE g.parent_goal_id=? AND g.level='employee'"                            " AND e.manager_id=? AND e.company_id=?",
                            conn, params=(dg['id'], username, company_id)
                        )
        ...
        with tab_emp:
            ...
            emp_goals = pd.read_sql_query(
                ...
                "AND g.manager_id=? AND g.company_id=? "
                "ORDER BY g.id",
                conn, params=(eid, current_period, username, company_id)
            )
                        ...
                        with st.expander(f"{'✅' if gp>=100 else '🎯'} {g['title']} — {int(g['weight'])}% težine"):
                            ...
                            with tabs_g[1]:
                                with st.form(f"edit_eg_{gid}"):
                                    ...
                                    dept_ids_eg   = [None] + [r['id'] for _, r in dept_goals_mine.iterrows()]
                                    dept_labels_eg = ["-- Bez linka --"] + [
                                        f"{_status_badge(float(r['progress'] or 0))[0]} {r['title']} "
                                        f"({float(r['progress'] or 0):.0f}%) #{r['id']}"
                                        for _, r in dept_goals_mine.iterrows()
                                    ]
                                    curr_idx2 = 0
                                    if g['parent_goal_id']:
                                        for i2, did_eg in enumerate(dept_ids_eg):
                                            if did_eg == g['parent_goal_id']:
                                                curr_idx2 = i2; break
                                    sel_idx2 = st.selectbox(
                                        "Cilj odjela:", range(len(dept_labels_eg)),
                                        format_func=lambda i: dept_labels_eg[i],
                                        index=curr_idx2, key=f"edit_dept_eg_{gid}"
                                    )

                                    ce1, ce2 = st.columns(2)
                                    if ce1.form_submit_button("💾 Spremi"):
                                        pid_e2 = dept_ids_eg[sel_idx2]
                                        conn.execute(
                                            "UPDATE goals SET title=?,weight=?,description=?,parent_goal_id=? WHERE id=? AND manager_id=?",
                                            (nt_e, nw_e, nd_e, pid_e2, gid, username)
                                        )
                                ...
                                if st.session_state.get(f"cdel_eg_{gid}"):
                                    ...
                                    if cyd.button("Da, obriši", key=f"yeg_{gid}"):
                                        ...
                                        conn.execute("DELETE FROM goals WHERE id=? AND manager_id=?", (gid, username))
                                        conn.commit()
