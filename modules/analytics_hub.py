...
def render_interactive_9box(company_id: int, current_period: str):
    ...
    try:
        ...
        # ── Snail Trail odabranog zaposlenika ─────────────────
        st.markdown("---")
        st.markdown("#### 🐌 Snail Trail — razvojni put zaposlenika")
        st.caption("Odaberi zaposlenika da vidiš kretanje kroz 9-box matricu kroz zadnje periode.")

        emp_opts = ["-- Odaberi zaposlenika --"] + sorted(
            display_df["ime_prezime"].tolist() if not display_df.empty
            else df["ime_prezime"].tolist()
        )
        sel_emp_trail = st.selectbox(
            "Zaposlenik:", emp_opts, key="nbox_trail_sel"
        )

        if sel_emp_trail != "-- Odaberi zaposlenika --":
            # Dohvati kadrovski_broj
            emp_row = df[df["ime_prezime"] == sel_emp_trail]
            if not emp_row.empty:
                trail_kid = emp_row.iloc[0]["kadrovski_broj"]

                # Povijest svih zaključanih procjena za tog zaposlenika
                trail_hist = pd.read_sql_query(
                    """SELECT period, avg_performance, avg_potential
                       FROM evaluations
                       WHERE kadrovski_broj=? AND company_id=?
                         AND is_self_eval=0 AND status='Submitted'
                       ORDER BY period ASC
                       LIMIT 6""",
                    conn, params=(trail_kid, company_id)
                )

                if trail_hist.empty or len(trail_hist) < 1:
                    st.info(f"📭 Nema zaključanih procjena za {sel_emp_trail}.")
                else:
                    # Kopiraj 9-box pozadinu + dodaj trail liniju
                    trail_fig = go.Figure()

                    # Pozadinski kvadranti (isti kao glavni graf)
                    _trail_zones = [
                        (0.5, 2.5, 0.5, 2.5, "#ffebee", "Potrebno poboljšanje"),
                        (2.5, 4.0, 0.5, 2.5, "#fff8e1", ""),
                        (4.0, 5.5, 0.5, 2.5, "#e8f5e9", ""),
                        (0.5, 2.5, 2.5, 4.0, "#fff8e1", ""),
                        (2.5, 4.0, 2.5, 4.0, "#f3e5f5", "Pouzdan suradnik"),
                        (4.0, 5.5, 2.5, 4.0, "#e8f5e9", ""),
                        (0.5, 2.5, 4.0, 5.5, "#e3f2fd", "Talent u razvoju"),
                        (2.5, 4.0, 4.0, 5.5, "#e8f5e9", ""),
                        (4.0, 5.5, 4.0, 5.5, "#fff9c4", "⭐ Top Talent"),
                    ]
                    for x0, x1, y0, y1, fill, label in _trail_zones:
                        trail_fig.add_shape(
                            type="rect", x0=x0, x1=x1, y0=y0, y1=y1,
                            fillcolor=fill, opacity=0.5,
                            line=dict(color="#ddd", width=1)
                        )
                        if label:
                            trail_fig.add_annotation(
                    ...
                    # Trail linija
                    trail_fig.add_trace(go.Scatter(
                        x=trail_hist["avg_performance"],
                        y=trail_hist["avg_potential"],
                        mode="lines+markers+text",
                        text=trail_hist["period"],
                        textposition="top center",
                        textfont=dict(size=10),
                        marker=dict(
                            size=[10 + i*3 for i in range(len(trail_hist))],
                            color=list(range(len(trail_hist))),
                            colorscale="Blues",
                            showscale=False,
                        ...
                        hovertemplate=(
                            "<b>%{text}</b><br>"
                            "Učinak: %{x:.2f}<br>"
                            "Potencijal: %{y:.2f}<extra></extra>"
                        )
                    ))

                    # Strelica na zadnjoj točki
                    if len(trail_hist) >= 2:
                        last  = trail_hist.iloc[-1]
                        prev  = trail_hist.iloc[-2]
                        trail_fig.add_annotation(
                            x=float(last["avg_performance"]),
                            y=float(last["avg_potential"]),
                            ax=float(prev["avg_performance"]),
                            ay=float(prev["avg_potential"]),
                            axref="x", ayref="y",
                            xref="x", yref="y",
                            showarrow=True,
                            arrowhead=3, arrowsize=1.5,
                            arrowcolor="#1565C0", arrowwidth=2
                        )

                    trail_fig.update_layout(
                        title=f"Razvojni put: {sel_emp_trail}",
                    ...
                    trail_fig.add_vline(x=2.5, line_dash="dot", line_color="#ccc")
                    trail_fig.add_vline(x=4.0, line_dash="dot", line_color="#ccc")
                    trail_fig.add_hline(y=2.5, line_dash="dot", line_color="#ccc")
                    trail_fig.add_hline(y=4.0, line_dash="dot", line_color="#ccc")

                    st.plotly_chart(trail_fig, use_container_width=True)

                    # Tekstualni sažetak trenda
                    if len(trail_hist) >= 2:
                        first_p = trail_hist.iloc[0]["avg_performance"]
                        last_p  = trail_hist.iloc[-1]["avg_performance"]
                        first_pot = trail_hist.iloc[0]["avg_potential"]
                        last_pot  = trail_hist.iloc[-1]["avg_potential"]
                        dp = last_p   - first_p
                        dpot = last_pot - first_pot
                        p_trend   = "↑" if dp   > 0.2 else ("↓" if dp   < -0.2 else "→")
                        pot_trend = "↑" if dpot > 0.2 else ("↓" if dpot < -0.2 else "→")
                        st.markdown(
                            f"**Trend:** Učinak {p_trend} ({first_p:.1f} → {last_p:.1f})  "
                            f"| Potencijal {pot_trend} ({first_pot:.1f} → {last_pot:.1f})  "
                            f"| {len(trail_hist)} perioda praćenja"
                        )
