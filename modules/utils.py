...
# --- STATUS PILL KOMPONENTA ---
# Mapiranje statusa → (emoji, boja pozadine, boja teksta)
_STATUS_PILL_MAP = {
    # Evaluacije
    "Submitted":  ("🔒", "#1b5e20", "#e8f5e9"),
    "Draft":      ("✏️", "#e65100", "#fff3e0"),
    "Locked":     ("🔒", "#1b5e20", "#e8f5e9"),
    # IDP / Planovi
    "Active":     ("🟢", "#1b5e20", "#e8f5e9"),
    "Approved":   ("✅", "#0d47a1", "#e3f2fd"),
    "Archived":   ("📦", "#4a4a4a", "#f5f5f5"),
    # Ciljevi
    "On Track":   ("🎯", "#1b5e20", "#e8f5e9"),
    "At Risk":    ("⚠️",  "#e65100", "#fff3e0"),
    "Behind":     ("🔴", "#b71c1c", "#ffebee"),
    "Completed":  ("✅", "#0d47a1", "#e3f2fd"),
    # Fallback
    "Nije kreiran": ("⚪", "#555555", "#f5f5f5"),
    "N/A":          ("—",  "#555555", "#f5f5f5"),
}

def render_status_pill(status: str, size: str = "normal") -> str:
    """
    Vraća HTML string s vizualno atraktivnom statusnom oznakom (pill).
    Koristi se s st.markdown(..., unsafe_allow_html=True).

    Parametri:
        status : tekstualni status (npr. 'Draft', 'Submitted', 'On Track')
        size   : 'small' | 'normal' | 'large'

    Primjer:
        st.markdown(render_status_pill("Submitted"), unsafe_allow_html=True)
    """
    sizes = {"small": "11px", "normal": "12px", "large": "14px"}
    padding = {"small": "2px 8px", "normal": "3px 10px", "large": "5px 14px"}
    fs = sizes.get(size, "12px")
    pd_val = padding.get(size, "3px 10px")

    icon, bg, fg = _STATUS_PILL_MAP.get(
        status, ("●", "#888888", "#f0f0f0")
    )
    return (
        f'<span style="display:inline-flex;align-items:center;gap:4px;'
        f'background:{bg};color:{fg};font-size:{fs};font-weight:600;'
        f'padding:{pd_val};border-radius:99px;white-space:nowrap;">'
        f'{icon} {status}</span>'
    )
