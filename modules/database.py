...
def init_db():
    ...
    try:
        ...
        try:
            c.execute("ALTER TABLE recognitions ADD COLUMN category TEXT DEFAULT 'Općenito'")
        except: pass
        ...
        c.execute('CREATE TABLE IF NOT EXISTS recognitions (id INTEGER PRIMARY KEY AUTOINCREMENT, sender_id TEXT, receiver_id TEXT, message TEXT, category TEXT DEFAULT "Općenito", timestamp TEXT, company_id INTEGER)')
        ...
        # ── V1.2: 360° PEER FEEDBACK TABLICE ─────────────────────────────────
        c.execute("""CREATE TABLE IF NOT EXISTS peer_nominations (
            id           INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            created_at   TEXT,
            UNIQUE(period, target_id, evaluator_id, company_id)
        )""")
        c.execute("""CREATE TABLE IF NOT EXISTS peer_feedback_results (
            id            INTEGER PRIMARY KEY AUTOINCREMENT,
            nomination_id INTEGER NOT NULL,
            competency    TEXT    NOT NULL,
            score         INTEGER CHECK(score BETWEEN 1 AND 5),
            comment       TEXT,
            FOREIGN KEY (nomination_id) REFERENCES peer_nominations(id)
                ON DELETE CASCADE
        )""")
        for _mig in [
            "ALTER TABLE periods ADD COLUMN peer_deadline TEXT",
            "ALTER TABLE peer_nominations ADD COLUMN token TEXT UNIQUE",
            "ALTER TABLE peer_nominations ADD COLUMN manager_id TEXT",
        ]:
            try:
                c.execute(_mig)
            except Exception:
